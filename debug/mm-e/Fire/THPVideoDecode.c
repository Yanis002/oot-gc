/*
    Compile unit: c:\HOMEBOY\STEPHEN\Emulator\Fire\THPVideoDecode.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x80013CC8 -> 0x800141E8
*/

#include "types.h"

// size = 0x4, address = 0x8012FDA0
...
} __anon_0x1A517; // size = 0x318

// size = 0x318, address = 0x800F53B0
static struct OSThread VideoDecodeThread;

// size = 0x1000, address = 0x800F56C8
...
} __anon_0x1AF94; // size = 0x3C

typedef struct __anon_0x1B06C {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x1B06C; // size = 0x30

typedef struct __anon_0x1B298 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x1B298; // size = 0x14

typedef struct __anon_0x1B300 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x1B300; // size = 0xC

typedef struct __anon_0x1B380 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x1B380; // size = 0x10

typedef struct __anon_0x1B43A {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x1B43A; // size = 0x10

typedef struct __anon_0x1B4F0 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x1B4F0; // size = 0xC

typedef struct __anon_0x1B5E0 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x1B06C header;
    /* 0x06C */ struct __anon_0x1B298 compInfo;
    /* 0x080 */ struct __anon_0x1B300 videoInfo;
    /* 0x08C */ struct __anon_0x1B380 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x1B43A* dispTextureSet;
    /* 0x0FC */ struct __anon_0x1B4F0* playAudioBuffer;
    /* 0x100 */ struct __anon_0x1BBE8 readBuffer[10];
    /* 0x178 */ struct __anon_0x1B43A textureSet[3];
    /* 0x1A8 */ struct __anon_0x1B4F0 audioBuffer[3];
} __anon_0x1B5E0; // size = 0x1D0

// size = 0x1D0, address = 0x800F2A40
struct __anon_0x1B5E0 ActivePlayer;

typedef struct __anon_0x1BBE8 {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x1BBE8; // size = 0xC

// Range: 0x800141B8 -> 0x800141E8
...
// Range: 0x80013FF0 -> 0x80014110
static void VideoDecode(struct __anon_0x1BBE8* readBuffer) {
    // Parameters
    // struct __anon_0x1BBE8* readBuffer; // r24

    // Local variables
    struct __anon_0x1B43A* textureSet; // r28
    u32 i; // r27
    u32* compSizePtr; // r26
    u8* ptr; // r25
    s32 old; // r3
...
// Range: 0x80013EBC -> 0x80013FF0
static void* VideoDecoderForOnMemory(void* ptr) {
    // Parameters
    // void* ptr; // r1+0x8

    // Local variables
    struct __anon_0x1BBE8 readBuffer; // r1+0x10
    s32 old; // r3
    s32 tmp; // r4
    s32 size; // r29
    s32 readFrame; // r28
...
// Range: 0x80013DE0 -> 0x80013EBC
static void* VideoDecoder() {
    // Local variables
    struct __anon_0x1BBE8* readBuffer; // r28
    s32 old; // r3
