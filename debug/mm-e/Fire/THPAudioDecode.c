/*
    ...
    Language: C89
    Code range: 0x80011B44 -> 0x80011EE0
*/

#include "types.h"

// size = 0x4, address = 0x8012FD80
...
} __anon_0x14703; // size = 0x318

// size = 0x318, address = 0x800F2C10
static struct OSThread AudioDecodeThread;

// size = 0x1000, address = 0x800F2F28
...
} __anon_0x151B7; // size = 0x3C

typedef struct __anon_0x1528F {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x1528F; // size = 0x30

typedef struct __anon_0x154BB {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x154BB; // size = 0x14

typedef struct __anon_0x15523 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x15523; // size = 0xC

typedef struct __anon_0x155A3 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x155A3; // size = 0x10

typedef struct __anon_0x1565D {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x1565D; // size = 0x10

typedef struct __anon_0x15713 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x15713; // size = 0xC

typedef struct __anon_0x15803 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x1528F header;
    /* 0x06C */ struct __anon_0x154BB compInfo;
    /* 0x080 */ struct __anon_0x15523 videoInfo;
    /* 0x08C */ struct __anon_0x155A3 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x1565D* dispTextureSet;
    /* 0x0FC */ struct __anon_0x15713* playAudioBuffer;
    /* 0x100 */ struct __anon_0x15D8C readBuffer[10];
    /* 0x178 */ struct __anon_0x1565D textureSet[3];
    /* 0x1A8 */ struct __anon_0x15713 audioBuffer[3];
} __anon_0x15803; // size = 0x1D0

// size = 0x1D0, address = 0x800F2A40
struct __anon_0x15803 ActivePlayer;

typedef struct __anon_0x15D8C {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x15D8C; // size = 0xC

// Range: 0x80011EB0 -> 0x80011EE0
...
// Range: 0x80011D30 -> 0x80011E08
static void AudioDecode(struct __anon_0x15D8C* readBuffer) {
    // Parameters
    // struct __anon_0x15D8C* readBuffer; // r1+0x8

    // Local variables
    struct __anon_0x15713* audioBuffer; // r30
    u32 i; // r1+0x8
    u32 sample; // r1+0x8
    u32* compSizePtr; // r29
    u8* ptr; // r28
...
// Range: 0x80011C80 -> 0x80011D30
static void* AudioDecoderForOnMemory(void* ptr) {
    // Parameters
    // void* ptr; // r1+0x8

    // Local variables
    struct __anon_0x15D8C readBuffer; // r1+0x10
    s32 tmp; // r4
    s32 size; // r28
    s32 readFrame; // r31
...
// Range: 0x80011C58 -> 0x80011C80
static void* AudioDecoder() {
    // Local variables
    struct __anon_0x15D8C* readBuffer; // r31
}
