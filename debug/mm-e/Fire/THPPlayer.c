/*
    Compile unit: c:\HOMEBOY\STEPHEN\Emulator\Fire\THPPlayer.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x80010638 -> 0x80011B44
*/

#include "types.h"

// size = 0x100, address = 0x800E1780
static u16 VolumeTable[128];

// size = 0x4, address = 0x8012FD60
static s32 Initialized;

// size = 0x40, address = 0x800F24A0
...
typedef struct OSMessageQueue {
    ...
    /* 0x18 */ s32 firstIndex;
    /* 0x1C */ s32 usedCount;
} __anon_0x1196B; // size = 0x20

// size = 0x20, address = 0x800F24E0
static struct OSMessageQueue PrepareReadyQueue;

// size = 0x20, address = 0x800F2500
static struct OSMessageQueue UsedTextureSetQueue;

// size = 0x4, address = 0x8012FD64
static void* PrepareReadyMessage;

// size = 0xC, address = 0x800F2520
static void* UsedTextureSetMessage[3];

// size = 0x4, address = 0x8012FD68
static void (* OldVIPostCallback)(u32);

// size = 0x500, address = 0x800F2540
static s16 SoundBuffer[2][320];

// size = 0x4, address = 0x8012FD6C
static s32 SoundBufferIndex;

// size = 0x4, address = 0x8012FD70
static void (* OldAIDCallback)();

// size = 0x4, address = 0x8012FD74
static s16* LastAudioBuffer;

// size = 0x4, address = 0x8012FD78
static s16* CurAudioBuffer;

// size = 0x4, address = 0x8012FD7C
...
} __anon_0x12102; // size = 0x3C

typedef struct __anon_0x121DA {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x121DA; // size = 0x30

typedef struct __anon_0x12406 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x12406; // size = 0x14

typedef struct __anon_0x1246E {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x1246E; // size = 0xC

typedef struct __anon_0x124EE {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x124EE; // size = 0x10

typedef struct __anon_0x125A8 {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x125A8; // size = 0x10

typedef struct __anon_0x1265E {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x1265E; // size = 0xC

typedef struct __anon_0x126E8 {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x126E8; // size = 0xC

typedef struct __anon_0x127D3 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x121DA header;
    /* 0x06C */ struct __anon_0x12406 compInfo;
    /* 0x080 */ struct __anon_0x1246E videoInfo;
    /* 0x08C */ struct __anon_0x124EE audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x125A8* dispTextureSet;
    /* 0x0FC */ struct __anon_0x1265E* playAudioBuffer;
    /* 0x100 */ struct __anon_0x126E8 readBuffer[10];
    /* 0x178 */ struct __anon_0x125A8 textureSet[3];
    /* 0x1A8 */ struct __anon_0x1265E audioBuffer[3];
} __anon_0x127D3; // size = 0x1D0

// size = 0x1D0, address = 0x800F2A40
struct __anon_0x127D3 ActivePlayer;

typedef enum __anon_0x1364D {
...
typedef enum __anon_0x13799 {
    VI_XFBMODE_SF = 0,
    VI_XFBMODE_DF = 1,
} __anon_0x13799;

typedef struct _GXRenderModeObj {
    /* 0x00 */ enum __anon_0x1364D viTVmode;
    /* 0x04 */ u16 fbWidth;
    /* 0x06 */ u16 efbHeight;
    /* 0x08 */ u16 xfbHeight;
    /* 0x0A */ u16 viXOrigin;
    ...
    /* 0x0E */ u16 viWidth;
    /* 0x10 */ u16 viHeight;
    /* 0x14 */ enum __anon_0x13799 xFBmode;
    /* 0x18 */ u8 field_rendering;
    /* 0x19 */ u8 aa;
    /* 0x1A */ u8 sample_pattern[12][2];
    /* 0x32 */ u8 vfilter[7];
} __anon_0x1384C; // size = 0x3C
...
// Erased
static s32 THPPlayerGetAudioInfo(struct __anon_0x124EE* audioInfo) {
    // Parameters
    // struct __anon_0x124EE* audioInfo; // r3
...
// Erased
static s32 THPPlayerGetVideoInfo(struct __anon_0x1246E* videoInfo) {
    // Parameters
    // struct __anon_0x1246E* videoInfo; // r3

    // References
    // -> struct __anon_0x127D3 ActivePlayer;
}

// Range: 0x800114C4 -> 0x80011590
...
// Range: 0x80011334 -> 0x800113A0
static s32 ProperTimingForStart() {
    // References
    // -> struct __anon_0x127D3 ActivePlayer;
}

// Range: 0x80011098 -> 0x80011334
static void PlayControl(u32 retraceCount) {
    ...
    s32 diff; // r1+0x8
    s32 frameNumber; // r4
    struct __anon_0x125A8* textureSet; // r29
...
// Range: 0x8001103C -> 0x80011098
s32 THPPlayerPlay() {
    // References
    // -> struct __anon_0x127D3 ActivePlayer;
}

// Range: 0x80010DC8 -> 0x8001103C
...
// Range: 0x80010CCC -> 0x80010D98
static void InitAllMessageQueue() {
    // Local variables
    s32 i; // r28
    struct __anon_0x126E8* readBuffer; // r3
    struct __anon_0x125A8* textureSet; // r3
    struct __anon_0x1265E* audioBuffer; // r3
