/*
    ...
    Language: C++
    Code range: 0x800126E0 -> 0x80013CC8
*/

#include "types.h"

// size = 0x4, address = 0x8012FD88
static s32 ReadThreadCreated;

typedef struct OSThreadQueue {
    /* 0x0 */ struct OSThread* head;
    /* 0x4 */ struct OSThread* tail;
} __anon_0x16BD3; // size = 0x8
...
} __anon_0x17398; // size = 0x318

// size = 0x318, address = 0x800F4058
static struct OSThread ReadThread;

// size = 0x1000, address = 0x800F4370
static u8 ReadThreadStack[4096];

// size = 0x40, address = 0x800F5370
static f32 gOrthoMtx[4][4];

// size = 0x4, address = 0x8012FD8C
s32 gMovieErrorToggle;

// size = 0x4, address = 0x8012FD90
static s32 toggle$185;

// size = 0x4, address = 0x8012FD94
static s32 gbReset;

// size = 0x4, address = 0x8012FD98
...
} __anon_0x17DC4; // size = 0x3C

typedef struct __anon_0x17E9C {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x17E9C; // size = 0x30

typedef struct __anon_0x180C8 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x180C8; // size = 0x14

typedef struct __anon_0x18130 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x18130; // size = 0xC

typedef struct __anon_0x181B0 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x181B0; // size = 0x10

typedef struct __anon_0x1826A {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x1826A; // size = 0x10

typedef struct __anon_0x18320 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x18320; // size = 0xC

typedef struct __anon_0x183AA {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x183AA; // size = 0xC

typedef struct __anon_0x18495 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x17E9C header;
    /* 0x06C */ struct __anon_0x180C8 compInfo;
    /* 0x080 */ struct __anon_0x18130 videoInfo;
    /* 0x08C */ struct __anon_0x181B0 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x1826A* dispTextureSet;
    /* 0x0FC */ struct __anon_0x18320* playAudioBuffer;
    /* 0x100 */ struct __anon_0x183AA readBuffer[10];
    /* 0x178 */ struct __anon_0x1826A textureSet[3];
    /* 0x1A8 */ struct __anon_0x18320 audioBuffer[3];
} __anon_0x18495; // size = 0x1D0

// size = 0x1D0, address = 0x800F2A40
struct __anon_0x18495 ActivePlayer;
...
typedef struct PADStatus {
    ...
    /* 0x9 */ u8 analogB;
    /* 0xA */ s8 err;
} __anon_0x18C5E; // size = 0xC

typedef struct __anon_0x18DEE {
...
} __anon_0x18DEE; // size = 0x1E

// size = 0x78, address = 0x8012CDD8
struct __anon_0x18DEE DemoPad[4];

typedef enum __anon_0x191B0 {
    M_M_NONE = -1,
...
// size = 0x0, address = 0x8012FCF8
u8 gcoverOpen[];

// size = 0x0, address = 0x8012FD08
u8 gwrongDisk[];

// size = 0x0, address = 0x8012FD0C
u8 greadingDisk[];

// size = 0x0, address = 0x8012FD00
u8 gretryErr[];

// size = 0x0, address = 0x8012FD04
u8 gfatalErr[];

// size = 0x0, address = 0x8012FCFC
u8 gnoDisk[];

// size = 0x0, address = 0x800E0A20
s16 Vert_s16[];
...
typedef struct __anon_0x1999C {
    ...
    /* 0x8 */ char* data;
} __anon_0x1999C; // size = 0xC

typedef struct __anon_0x19A68 {
    /* 0x0 */ struct __anon_0x19741* textureHeader;
    /* 0x4 */ struct __anon_0x1999C* CLUTHeader;
} __anon_0x19A68; // size = 0x8

typedef struct __anon_0x19AD9 {
    /* 0x0 */ u32 versionNumber;
    /* 0x4 */ u32 numDescriptors;
    /* 0x8 */ struct __anon_0x19A68* descriptorArray;
} __anon_0x19AD9; // size = 0xC

typedef struct _GXTexObj {
    /* 0x0 */ u32 dummy[8];
} __anon_0x19B95; // size = 0x20
...
typedef struct _GXColor {
    ...
    /* 0x2 */ u8 b;
    /* 0x3 */ u8 a;
} __anon_0x19BDB; // size = 0x4

// Range: 0x800135F8 -> 0x80013CC8
s32 movieGXInit() {
    // Local variables
    s32 i; // r31
...
// Range: 0x800130DC -> 0x800135F8
s32 movieDrawImage(struct __anon_0x19AD9* tpl, s16 nX0, s16 nY0) {
    // Parameters
    // struct __anon_0x19AD9* tpl; // r25
    // s16 nX0; // r26
    // s16 nY0; // r27
...
// Range: 0x80012F18 -> 0x800130DC
s32 movieDrawErrorMessage(enum __anon_0x191B0 movieMessage) {
    // Parameters
    // enum __anon_0x191B0 movieMessage; // r1+0x8
...
// Range: 0x80012CB8 -> 0x80012F18
s32 movieDVDShowError(s32 nStatus) {
    // Parameters
    // s32 nStatus; // r1+0x8

    // Local variables
    enum __anon_0x191B0 nMessage; // r31
...
// Range: 0x800127A8 -> 0x800128D4
static void* Reader() {
    // Local variables
    struct __anon_0x183AA* readBuffer; // r27
    s32 offset; // r26
    s32 size; // r25
    s32 readFrame; // r24
