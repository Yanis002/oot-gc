/*
    ...
    Language: C++
    Code range: 0x80013D80 -> 0x8001C720
*/

#include "types.h"

// size = 0xA000, address = 0x80107B00
static char gMCardCardWorkArea[40960];

// size = 0x4, address = 0x80141140
s32 currentIdx;

// size = 0x4, address = 0x80141144
static s32 yes$774;

typedef enum __anon_0x1C130 {
    MC_M_NONE = 0,
    MC_M_LD01 = 1,
    MC_M_LD02 = 2,
    MC_M_LD03 = 3,
...
// size = 0x4, address = 0x80141148
static enum __anon_0x1C130 prevMenuEntry$775;

// size = 0x4, address = 0x8014114C
static enum __anon_0x1C130 nextMenuEntry$776;

// size = 0x4, address = 0x80141150
s32 mcardNewStart;

// size = 0x4, address = 0x80141154
s32 mcardSaveDisplay;

// size = 0x4, address = 0x80141158
s32 mcardOneTime;

// size = 0x4, address = 0x80140828
static s32 toggle$1032;

// size = 0x4, address = 0x8014115C
static s32 toggle2$1033;

// size = 0x4, address = 0x80141160
static s32 checkFailCount$1624;
...
typedef struct OSCalendarTime {
    ...
    /* 0x20 */ s32 msec;
    /* 0x24 */ s32 usec;
} __anon_0x1C5AB; // size = 0x28

// size = 0x28, address = 0x80111B00
struct OSCalendarTime gDate;

// size = 0x4, address = 0x80141164
s32 bWrite2Card;

// size = 0x28, address = 0x80111B28
s32 bNoWriteInCurrentFrame[10];

typedef struct __anon_0x1C7D7 {
...
typedef struct CARDFileInfo {
    ...
    /* 0x10 */ u16 iBlock;
    /* 0x12 */ u16 __padding;
} __anon_0x1C97A; // size = 0x14

typedef struct __anon_0x1CA72 {
    /* 0x000 */ s32 currentGame;
    /* 0x004 */ s32 fileSize;
    /* 0x008 */ char name[33];
    /* 0x02C */ s32 numberOfGames;
    /* 0x030 */ struct __anon_0x1C7D7 game;
...
} __anon_0x1CA72; // size = 0x35C

typedef enum __anon_0x1CC88 {
    MC_E_NONE = 0,
    MC_E_BUSY = 1,
    MC_E_WRONGDEVICE = 2,
    MC_E_NOCARD = 3,
...
typedef struct _MCARD {
    /* 0x000 */ struct __anon_0x1CA72 file;
    /* 0x35C */ enum __anon_0x1CC88 error;
    /* 0x360 */ s32 slot;
...
} __anon_0x1CF2C; // size = 0x7B8

// size = 0x7B8, address = 0x80111B50
struct _MCARD mCard;

typedef enum __anon_0x1D4B2 {
    MC_C_NONE = 0,
    MC_C_CONTINUE = 1,
    MC_C_IPL = 2,
    MC_C_GO_TO_GAME = 3,
...
} __anon_0x1D7C0; // size = 0x174

// size = 0x174, address = 0x8013B910
struct __anon_0x1D7C0 gSystemRomConfigurationList[1];

typedef enum __anon_0x1DC8C {
    SM_NONE = -1,
    SM_RUNNING = 0,
    SM_STOPPED = 1,
} __anon_0x1DC8C;

typedef struct __anon_0x1DCF4 {
    /* 0x0 */ s32 nSize;
    /* 0x4 */ s32 nOffsetRAM;
    /* 0x8 */ s32 nOffsetROM;
    /* 0xC */ s32 (* pCallback)();
} __anon_0x1DCF4; // size = 0x10

typedef enum __anon_0x1DDA5 {
    SRT_NONE = -1,
    SRT_MARIO = 0,
    SRT_WAVERACE = 1,
    SRT_MARIOKART = 2,
...
typedef struct __anon_0x1E048 {
    /* 0x00 */ void* pFrame;
    /* 0x04 */ void* pSound;
    /* 0x08 */ s32 bException;
    /* 0x0C */ enum __anon_0x1DC8C eMode;
    /* 0x10 */ struct __anon_0x1DCF4 romCopy;
    /* 0x20 */ enum __anon_0x1DDA5 eTypeROM;
    /* 0x24 */ void* apObject[16];
    /* 0x68 */ u64 nAddressBreak;
    /* 0x70 */ enum __anon_0x1DF08 storageDevice;
    /* 0x74 */ u8 anException[16];
    /* 0x84 */ s32 bJapaneseVersion;
} __anon_0x1E048; // size = 0x88

// size = 0x4, address = 0x801410E0
struct __anon_0x1E048* gpSystem;

typedef struct _GXTexObj {
    /* 0x0 */ u32 dummy[8];
} __anon_0x1EAD8; // size = 0x20
...
typedef struct __anon_0x2128E {
    /* 0x0 */ u16 numEntries;
    /* 0x2 */ u8 unpacked;
    /* 0x3 */ u8 pad8;
    /* 0x4 */ enum _GXTlutFmt format;
    /* 0x8 */ char* data;
} __anon_0x2128E; // size = 0xC

typedef struct __anon_0x2135A {
    /* 0x0 */ struct __anon_0x21033* textureHeader;
    /* 0x4 */ struct __anon_0x2128E* CLUTHeader;
} __anon_0x2135A; // size = 0x8

typedef struct __anon_0x213CB {
    /* 0x0 */ u32 versionNumber;
    /* 0x4 */ u32 numDescriptors;
    /* 0x8 */ struct __anon_0x2135A* descriptorArray;
} __anon_0x213CB; // size = 0xC

// Erased
static void mcardUnpackTexPalette(struct __anon_0x213CB* pal) {
    // Parameters
    // struct __anon_0x213CB* pal; // r1+0x0

    // Local variables
    u16 i; // r4
}

// Range: 0x8001C57C -> 0x8001C720
...
// Range: 0x800197F0 -> 0x8001996C
s32 mcardReInit(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r30
}

// Range: 0x80019798 -> 0x800197F0
s32 mcardInit(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r31
}

// Range: 0x80019374 -> 0x80019798
...
// Range: 0x80017D1C -> 0x80017EAC
s32 mcardFileErase(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r30
}

// Range: 0x80017B74 -> 0x80017D1C
...
// Range: 0x80017AF0 -> 0x80017B74
s32 mcardGameRelease(struct _MCARD* pMCard) {
    // Parameters
    // struct _MCARD* pMCard; // r31
}

// Range: 0x80017AC0 -> 0x80017AF0
...
// Range: 0x8001711C -> 0x80017AC0
s32 mcardMenu(struct _MCARD* pMCard, enum __anon_0x1C130 menuEntry, enum __anon_0x1D4B2* pCommand) {
    // Parameters
    // struct _MCARD* pMCard; // r29
    // enum __anon_0x1C130 menuEntry; // r4
    // enum __anon_0x1D4B2* pCommand; // r30
...
// Range: 0x8001703C -> 0x8001711C
s32 mcardOpenError(struct _MCARD* pMCard, enum __anon_0x1D4B2* pCommand) {
    // Parameters
    // struct _MCARD* pMCard; // r3
    // enum __anon_0x1D4B2* pCommand; // r4
}

// Range: 0x80016F5C -> 0x8001703C
s32 mcardOpenDuringGameError(struct _MCARD* pMCard, enum __anon_0x1D4B2* pCommand) {
    // Parameters
    // struct _MCARD* pMCard; // r3
    // enum __anon_0x1D4B2* pCommand; // r4
}
...
// Erased
static s32 mcardGetError(struct _MCARD* pMCard, enum __anon_0x1CC88* pMCardError) {
    // Parameters
    // struct _MCARD* pMCard; // r1+0x0
    // enum __anon_0x1CC88* pMCardError; // r1+0x4
}
...
s32 mcardOpen(struct _MCARD* pMCard, char* fileName, char* comment, char* icon, char* banner, char* gameName, s32* defaultConfiguration, s32 fileSize, s32 gameSize) {
    ...
    // Local variables
    s32 i; // r24
    enum __anon_0x1D4B2 command; // r1+0x34
...
s32 mcardOpenDuringGame(struct _MCARD* pMCard) {
    ...
    // Local variables
    s32 i; // r28
    enum __anon_0x1D4B2 command; // r1+0x18
    s32 loadToggle; // r27
...
s32 mcardStore(struct _MCARD* pMCard) {
    ...
    s32 bufferOffset; // r4
    enum __anon_0x1D4B2 command; // r1+0x4C8
