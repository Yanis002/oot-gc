/*
    ...
    Language: C89
    Code range: 0x800101D0 -> 0x800116DC
*/

#include "types.h"

// size = 0x100, address = 0x800F2848
static u16 VolumeTable[128];

// size = 0x4, address = 0x801410F8
static s32 Initialized;

// size = 0x40, address = 0x80103880
...
typedef struct OSMessageQueue {
    ...
    /* 0x18 */ s32 firstIndex;
    /* 0x1C */ s32 usedCount;
} __anon_0x1186F; // size = 0x20

// size = 0x20, address = 0x801038C0
static struct OSMessageQueue PrepareReadyQueue;

// size = 0x20, address = 0x801038E0
static struct OSMessageQueue UsedTextureSetQueue;

// size = 0x4, address = 0x801410FC
static void* PrepareReadyMessage;

// size = 0xC, address = 0x80103900
static void* UsedTextureSetMessage[3];

// size = 0x4, address = 0x80141100
static void (* OldVIPostCallback)(u32);

// size = 0x500, address = 0x80103920
static s16 SoundBuffer[2][320];

// size = 0x4, address = 0x80141104
static s32 SoundBufferIndex;

// size = 0x4, address = 0x80141108
static void (* OldAIDCallback)();

// size = 0x4, address = 0x8014110C
static s16* LastAudioBuffer;

// size = 0x4, address = 0x80141110
static s16* CurAudioBuffer;

// size = 0x4, address = 0x80141114
...
} __anon_0x12006; // size = 0x3C

typedef struct __anon_0x120DE {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x120DE; // size = 0x30

typedef struct __anon_0x1230A {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x1230A; // size = 0x14

typedef struct __anon_0x12372 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x12372; // size = 0xC

typedef struct __anon_0x123F2 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x123F2; // size = 0x10

typedef struct __anon_0x124AC {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x124AC; // size = 0x10

typedef struct __anon_0x12562 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x12562; // size = 0xC

typedef struct __anon_0x125EC {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x125EC; // size = 0xC

typedef struct __anon_0x126D7 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x120DE header;
    /* 0x06C */ struct __anon_0x1230A compInfo;
    /* 0x080 */ struct __anon_0x12372 videoInfo;
    /* 0x08C */ struct __anon_0x123F2 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x124AC* dispTextureSet;
    /* 0x0FC */ struct __anon_0x12562* playAudioBuffer;
    /* 0x100 */ struct __anon_0x125EC readBuffer[10];
    /* 0x178 */ struct __anon_0x124AC textureSet[3];
    /* 0x1A8 */ struct __anon_0x12562 audioBuffer[3];
} __anon_0x126D7; // size = 0x1D0

// size = 0x1D0, address = 0x80103E20
struct __anon_0x126D7 ActivePlayer;

typedef enum __anon_0x13551 {
...
typedef enum __anon_0x1369D {
    VI_XFBMODE_SF = 0,
    VI_XFBMODE_DF = 1,
} __anon_0x1369D;

typedef struct _GXRenderModeObj {
    /* 0x00 */ enum __anon_0x13551 viTVmode;
    /* 0x04 */ u16 fbWidth;
    /* 0x06 */ u16 efbHeight;
    /* 0x08 */ u16 xfbHeight;
    /* 0x0A */ u16 viXOrigin;
    ...
    /* 0x0E */ u16 viWidth;
    /* 0x10 */ u16 viHeight;
    /* 0x14 */ enum __anon_0x1369D xFBmode;
    /* 0x18 */ u8 field_rendering;
    /* 0x19 */ u8 aa;
    /* 0x1A */ u8 sample_pattern[12][2];
    /* 0x32 */ u8 vfilter[7];
} __anon_0x13750; // size = 0x3C
...
// Erased
static s32 THPPlayerGetAudioInfo(struct __anon_0x123F2* audioInfo) {
    // Parameters
    // struct __anon_0x123F2* audioInfo; // r3
...
// Erased
static s32 THPPlayerGetVideoInfo(struct __anon_0x12372* videoInfo) {
    // Parameters
    // struct __anon_0x12372* videoInfo; // r3

    // References
    // -> struct __anon_0x126D7 ActivePlayer;
}

// Range: 0x8001105C -> 0x80011128
...
// Range: 0x80010ECC -> 0x80010F38
static s32 ProperTimingForStart() {
    // References
    // -> struct __anon_0x126D7 ActivePlayer;
}

// Range: 0x80010C30 -> 0x80010ECC
static void PlayControl(u32 retraceCount) {
    ...
    s32 diff; // r1+0x8
    s32 frameNumber; // r4
    struct __anon_0x124AC* textureSet; // r29
...
// Range: 0x80010BD4 -> 0x80010C30
s32 THPPlayerPlay() {
    // References
    // -> struct __anon_0x126D7 ActivePlayer;
}

// Range: 0x80010960 -> 0x80010BD4
...
// Range: 0x80010864 -> 0x80010930
static void InitAllMessageQueue() {
    // Local variables
    s32 i; // r28
    struct __anon_0x125EC* readBuffer; // r3
    struct __anon_0x124AC* textureSet; // r3
    struct __anon_0x12562* audioBuffer; // r3
