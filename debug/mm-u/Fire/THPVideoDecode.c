/*
    ...
    Language: C89
    Code range: 0x80013860 -> 0x80013D80
*/

#include "types.h"

// size = 0x4, address = 0x80141138
...
} __anon_0x1A41B; // size = 0x318

// size = 0x318, address = 0x80106790
static struct OSThread VideoDecodeThread;

// size = 0x1000, address = 0x80106AA8
...
} __anon_0x1AE98; // size = 0x3C

typedef struct __anon_0x1AF70 {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x1AF70; // size = 0x30

typedef struct __anon_0x1B19C {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x1B19C; // size = 0x14

typedef struct __anon_0x1B204 {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x1B204; // size = 0xC

typedef struct __anon_0x1B284 {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x1B284; // size = 0x10

typedef struct __anon_0x1B33E {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x1B33E; // size = 0x10

typedef struct __anon_0x1B3F4 {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x1B3F4; // size = 0xC

typedef struct __anon_0x1B4E4 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x1AF70 header;
    /* 0x06C */ struct __anon_0x1B19C compInfo;
    /* 0x080 */ struct __anon_0x1B204 videoInfo;
    /* 0x08C */ struct __anon_0x1B284 audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x1B33E* dispTextureSet;
    /* 0x0FC */ struct __anon_0x1B3F4* playAudioBuffer;
    /* 0x100 */ struct __anon_0x1BAEC readBuffer[10];
    /* 0x178 */ struct __anon_0x1B33E textureSet[3];
    /* 0x1A8 */ struct __anon_0x1B3F4 audioBuffer[3];
} __anon_0x1B4E4; // size = 0x1D0

// size = 0x1D0, address = 0x80103E20
struct __anon_0x1B4E4 ActivePlayer;

typedef struct __anon_0x1BAEC {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x1BAEC; // size = 0xC

// Range: 0x80013D50 -> 0x80013D80
...
// Range: 0x80013B88 -> 0x80013CA8
static void VideoDecode(struct __anon_0x1BAEC* readBuffer) {
    // Parameters
    // struct __anon_0x1BAEC* readBuffer; // r24

    // Local variables
    struct __anon_0x1B33E* textureSet; // r28
    u32 i; // r27
    u32* compSizePtr; // r26
    u8* ptr; // r25
    s32 old; // r3
...
// Range: 0x80013A54 -> 0x80013B88
static void* VideoDecoderForOnMemory(void* ptr) {
    // Parameters
    // void* ptr; // r1+0x8

    // Local variables
    struct __anon_0x1BAEC readBuffer; // r1+0x10
    s32 old; // r3
    s32 tmp; // r4
    s32 size; // r29
    s32 readFrame; // r28
...
// Range: 0x80013978 -> 0x80013A54
static void* VideoDecoder() {
    // Local variables
    struct __anon_0x1BAEC* readBuffer; // r28
    s32 old; // r3
